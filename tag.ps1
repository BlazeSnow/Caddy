...
try {
    # 1. 确定版本号：优先用 -Tag，否则从 VERSION 文件读取
    if (-not $Tag) {
        ...
        $Tag = 'v' + $version
        Write-Host "从 VERSION 文件读取到最新版本：$Tag"
    }
    ...
    Write-Host "  tag  ：$Tag"
    if ($Beta) {
        Write-Host '  类型 ：beta 测试版（触发 beta 镜像构建 + Prerelease）'
    }
    else {
        Write-Host '  类型 ：正式版（触发 GitHub Release）'
    }
