...
- 镜像会写入 `org.opencontainers.image.version` label（由 build job 以 build-arg 注入）：生产镜像用 `VERSION` 文件的正式版号；beta 镜像不携带正式版号——beta tag 触发时用 tag 名（如 `1.4.4-beta.1`），dev 分支触发时为 `dev`。同时写入 `caddy-version` 和 `plugin-version` label（本次构建的 Caddy 版本和插件版本），`docker inspect` 即可查看镜像构成
